...
<div class="col-sm-4 mb-5">
 ...
 if($result->num_rows > 0){
  while($row = $result->fetch_assoc()){
   ...
   echo '<h5 class="card-title">Request Info : ' . $row['name'] . '</h5>';
   echo '<p class="card-text">' . $row['email'] . '</p>';
   echo '<p class="card-text">Request Message: ' . $row['message'] . '</p>';
   echo '<p class="card-text">Request Phone: ' . $row['phone'] . '</p>';
